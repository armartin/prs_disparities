...
date(); all_phenos <- ldply(phenos, read_phenos, .parallel=T, .paropts=list(.packages=c('dplyr', 'tidyr'))); date()
...
date(); r2_combined <- ldply(phenos, function(x) { ldply(p_thresholds, function(y) { compute_r2(all_phenos, x, y) } ) }, 
                             .parallel=T, .paropts=list(.export = c("compute_r2", "bootstrap_CIs", "p_thresholds", "all_phenos", "covariates"))); date()
